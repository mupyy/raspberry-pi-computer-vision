...
def run(model: str, label: str, max_results: int, num_threads: int, camera_id: int, width: int, height: int) -> None:
  ...
  counter, counter2, fps, last_inference_start_time, time_per_infer = 0, 0, 0, 0, 0
  categories = []
  ...
  while cap.isOpened():
    ...
    for idx, category in enumerate(categories):
      ...
      cv2.putText(image, str(counter), (1,7), cv2.FONT_HERSHEY_PLAIN, _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)
      cv2.putText(image, str(counter2), (1,80), cv2.FONT_HERSHEY_PLAIN, _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)

    """if counter2 == 100:
        classifier.clear()"""
    #aperta tecla "P" para apagar a aferição anterior armazenada
